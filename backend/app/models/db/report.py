...
class Report(Base):
    ...
    user_id = Column(GUID, ForeignKey('users.id', ondelete='CASCADE'), index=True, nullable=True)
    company_id = Column(GUID, ForeignKey('companies.id'), index=True, nullable=True)
...
class ReportEvent(Base):
    ...
    report_id = Column(GUID, ForeignKey('reports.id'), index=True)
